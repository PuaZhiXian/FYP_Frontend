...
export class ApiCollectionComponent implements OnInit {
  ...
  createNewCollectionModalVisibility: boolean = true;
  ...
  previewApiCollectionId: string = "1";
  ...
  beforeUploadV2 = (file: NzUploadFile, fileList: NzUploadFile[]): boolean | Observable<boolean> => {
    // Perform any checks or modifications here before uploading
    console.log(file)
    return true;
  }
      ...
      this.apiCollectionService.createNewApiCollection(this.createNewCollectionForm.value)
        .subscribe((resp) => {
          if (resp.message) {
            this.message.success(resp.message);
            this.closeCreateNewCollectionModal();
            //TODO --  return preview api collection id
